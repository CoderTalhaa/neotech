  ...
  return (
    <div className="">
      {service.map((service, index) => {
        return <Card key={index} service={service} number={index + 1} />;
      })}
    </div>
  );
};

function Card({ service, number }) {
  const cardRef = useRef(null);
  const isInView = useInView(cardRef, { once: true, margin: "-300px" });
  ...
  return (
    <motion.div
      ...
      ref={cardRef}
      className={`relative flex w-full p-8 my-10  ${
        service.align === "left" ? "justify-end" : "justify-start"
      } `}
    >
      <motion.div
        whileHover={{ backgroundColor: "#0f2d3c90" }}
        className="bg-card-gradient border border-[#0f2d3c] rounded-lg px-6 py-16 max-w-[440px] relative "
      >
        <div className="text-[#30f3e3] ~text-base/4xl font-light font-zentry mb-2">
          0{number}
        </div>
        <h3 className="text-white ~text-xl/4xl font-medium mb-3">
          {service.name}
        </h3>
        <p className="text-gray-400 text-sm font-secondary leading-relaxed">
          {service.description}
        </p>
      </motion.div>
    </motion.div>
